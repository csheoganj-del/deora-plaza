...
ALTER TABLE public.orders ADD CONSTRAINT IF NOT EXISTS check_valid_type
CHECK ("type" IN ('dine-in', 'takeaway', 'delivery', 'room-service', 'bar'));

ALTER TABLE public.bills ADD CONSTRAINT IF NOT EXISTS check_valid_payment_status
CHECK ("paymentStatus" IN ('pending', 'paid', 'cancelled', 'refunded'));

ALTER TABLE public.bills ADD CONSTRAINT IF NOT EXISTS check_valid_payment_method
CHECK ("paymentMethod" IN ('cash', 'card', 'upi', 'online', 'credit'));

ALTER TABLE public.tables ADD CONSTRAINT IF NOT EXISTS check_valid_table_status
CHECK ("status" IN ('available', 'occupied', 'reserved', 'cleaning', 'maintenance'));

ALTER TABLE public.rooms ADD CONSTRAINT IF NOT EXISTS check_valid_room_status
CHECK ("status" IN ('available', 'occupied', 'maintenance', 'cleaning', 'out-of-order'));

ALTER TABLE public.bookings ADD CONSTRAINT IF NOT EXISTS check_valid_booking_status
CHECK ("status" IN ('confirmed', 'checked-in', 'checked-out', 'cancelled', 'no-show'));

ALTER TABLE public.bookings ADD CONSTRAINT IF NOT EXISTS check_valid_booking_type
CHECK ("type" IN ('hotel', 'event', 'meeting', 'party'));

ALTER TABLE public.bookings ADD CONSTRAINT IF NOT EXISTS check_valid_payment_status
CHECK ("paymentStatus" IN ('pending', 'partial', 'completed', 'refunded'));

ALTER TABLE public.discounts ADD CONSTRAINT IF NOT EXISTS check_valid_discount_type
CHECK ("type" IN ('percentage', 'fixed', 'buy-get'));

ALTER TABLE public.notifications ADD CONSTRAINT IF NOT EXISTS check_valid_priority
CHECK ("priority" IN ('low', 'normal', 'high', 'urgent'));

ALTER TABLE public.transactions ADD CONSTRAINT IF NOT EXISTS check_valid_transaction_type
CHECK ("type" IN ('sale', 'refund', 'expense', 'deposit', 'withdrawal', 'adjustment'));
...
COMMENT ON TABLE public.orders IS 'Core orders table with comprehensive timeline tracking and settlement management';
COMMENT ON TABLE public.bills IS 'Bills and invoices with payment tracking';
COMMENT ON TABLE public.notifications IS 'System notifications for kitchen, waiters, and managers';
COMMENT ON TABLE public.settlements IS 'Daily cash reconciliation and department settlements';
COMMENT ON TABLE public.audit_logs IS 'Security audit trail for compliance and forensic analysis';
COMMENT ON TABLE public.statistics IS 'Analytics and performance metrics storage';
COMMENT ON TABLE public.transactions IS 'Financial transaction tracking for accounting integration';
COMMENT ON TABLE public.rooms IS 'Hotel room inventory and management';
COMMENT ON TABLE public.bookings IS 'Hotel bookings and event reservations with payment tracking';

-- Insert initial business settings
INSERT INTO public.businessSettings ("id", "name", "address", "mobile")
VALUES ('main', 'DEORA POS', 'Business Address', '1234567890')
...
AND tablename IN (
  'users', 'orders', 'bills', 'menu_items', 'customers', 'tables', 'rooms',
  'bookings', 'inventory', 'businesssettings', 'counters', 'discounts',
  'notifications', 'settlements', 'department_settlements', 'transactions',
  'audit_logs', 'statistics'
)
ORDER BY tablename;
