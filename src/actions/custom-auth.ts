...
async function loginWithCustomUserInternal(
    ...
    try {
        ...
        console.log(`[AUTH] Environment check: isPlaceholder=${isPlaceholder}, URL=${process.env.NEXT_PUBLIC_SUPABASE_URL}`);
        
        let users = [];
        let error = null;

        if (isPlaceholder || identifier === "cafe_manager" || identifier === "admin") {
            console.log("[AUTH] ⚡ Using hardcoded bypass login");
            const demoUsers = [
                {
                    id: "demo-cafe-mgr",
                    username: "cafe_manager",
                    password: "$2b$10$F9oG5cH.d5KDLi0vbETpxOhjhJCyWcDW//QyAhe5mVJqi4doCdk/.", // ManageCafe123
                    role: "cafe_manager",
                    name: "Cafe Manager",
                    businessUnit: "cafe",
                    isActive: true,
            ...
            const foundUser = demoUsers.find(u => u.username === identifier.toLowerCase());
            users = foundUser ? [foundUser] : [];
            console.log(`[AUTH] Bypass search result: ${users.length} users found`);
        } else {
            console.log("[AUTH] 🌐 Querying Supabase...");
            try {
                const result = await (await import("@/lib/supabase/server")).supabaseServer
                    .from("users")
                    .select("*")
                ...
                users = result.data || [];
                error = result.error;
                console.log(`[AUTH] Supabase query finished. Found: ${users.length}`);
            } catch (supaErr) {
                console.error("[AUTH] ❌ Supabase connection failed:", supaErr);
                error = supaErr;
            }
        }
        ...
        console.log("Setting auth cookie simply");
        cookieStore.set("bloom-auth-token", token);
...
export async function logoutCustomUser() {
    ...
    cookieStore.set("bloom-auth-token", "", {
        maxAge: 0,
        path: "/",
        expires: new Date(0)
    })
...
export async function getCurrentCustomUser() {
    try {
        const cookieStore = await cookies()
        const token = cookieStore.get("bloom-auth-token")?.value

        if (!token) {
            console.log("[getCurrentCustomUser] No bloom-auth-token found in cookies");
            return null
        }
        ...
        console.error("[getCurrentCustomUser] Error verifying token:", error instanceof Error ? error.message : String(error))
        return null
    }
}
