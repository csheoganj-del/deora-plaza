...
export async function getGSTSummary(startDate?: Date, endDate?: Date, businessUnit?: string): Promise<{
  todayGst?: number;
  monthGst?: number;
  yearGst?: number;
  ...
  billCount?: number;
  averageGST?: number;
  byGSTRate?: Record<string, { count: number, subtotal: number, gst: number, total: number }>;
}> {
  try {
    // If dates provided, use legacy format for backward compatibility
...
export async function exportGSTData(
  ...
  businessUnit?: string
): Promise<{ success?: boolean; data?: any; error?: string; headers?: string[]; rows?: any[]; filename?: string }> {
  try {
    // Handle both Date objects and strings for backward compatibility
    ...
    if (format === 'json' || format === 'excel') {
      ...
      const reportResult = await generateGSTReport(startDateStr, endDateStr, businessUnit);
    ...
    const bills = await getGSTReport(
    ...
    const startDateFormatted = typeof startDate === 'string' ? startDate.split('T')[0] : startDate.toISOString().split('T')[0];
    const endDateFormatted = typeof endDate === 'string' ? endDate.split('T')[0] : endDate.toISOString().split('T')[0];
    
    return {
        headers,
        rows,
        filename: `GST_Report${unitSuffix}_${startDateFormatted}_to_${endDateFormatted}.csv`
    }
  } catch (error) {
...
// Validate GST number
export async function validateGSTIN(gstin: string): Promise<{ isValid: boolean; error?: string }> {
  if (!gstin) {
