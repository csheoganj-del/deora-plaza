...
export async function getTables(businessUnit?: string): Promise<any[]> {
    try {
        ...
        if (isPlaceholder) {
            console.log(`[TABLES] ⚡ Using mock tables for demo mode (${businessUnit || 'all'})`);
            const mockTables = [
                { id: 't1', tableNumber: '1', businessUnit: 'cafe', capacity: 4, status: 'available', customerCount: 0 },
                { id: 't2', tableNumber: '2', businessUnit: 'cafe', capacity: 2, status: 'occupied', customerCount: 2, currentOrderId: 'order-1' },
                { id: 't3', tableNumber: '3', businessUnit: 'cafe', capacity: 6, status: 'available', customerCount: 0 },
                { id: 't4', tableNumber: 'B1', businessUnit: 'bar', capacity: 2, status: 'available', customerCount: 0 },
                { id: 't5', tableNumber: 'B2', businessUnit: 'bar', capacity: 2, status: 'occupied', customerCount: 1, currentOrderId: 'order-2' },
            ];
            
            return businessUnit 
                ? mockTables.filter(t => t.businessUnit === businessUnit)
                : mockTables;
        }
