...
export async function getBills(businessUnit?: string, limit: number = 200) {
  try {
    let allBills: any[] = [];
    ...
    if (!businessUnit || businessUnit === 'cafe' || businessUnit === 'bar') {
      const filters = businessUnit
        ...
        : [];
      const bills = await queryDocuments("bills", filters, "createdAt", "desc", limit);
    ...
    if (!businessUnit || businessUnit === 'hotel' || businessUnit === 'garden') {
      ...
      const bookings = await queryDocuments("bookings", filters, "createdAt", "desc", limit);
