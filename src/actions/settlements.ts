...
export async function getInterDepartmentalOrders(filters?: {
  ...
  try {
    ...
    let query = supabaseServer
      ...
      .select(`
        *,
        ...
        customer:customers(*)
      `)
      .neq('fromDepartment', 'toDepartment'); // Inter-departmental only
    ...
    if (filters?.fromDepartment) {
      query = query.eq('fromDepartment', filters.fromDepartment);
    }
    if (filters?.toDepartment) {
      query = query.eq('toDepartment', filters.toDepartment);
    }
    if (filters?.status) {
