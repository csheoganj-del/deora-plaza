...
export default async function GardenGSTReportPage() {
    const session = await requireAuth()
    
    // Check if user has permission to access garden GST report
    const userRole = session.user.role
    const userBusinessUnit = session.user.businessUnit
    
    const allowedRoles = ['super_admin', 'owner', 'garden_manager']
    ...
    return (
        <div className="flex-1 space-y-4 p-8 pt-6">
            <GSTReportComponent businessUnit="garden" unitName="Garden" />
        </div>
    )
}
