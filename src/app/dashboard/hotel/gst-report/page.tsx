...
export default async function HotelGSTReportPage() {
    const session = await requireAuth()
    
    // Check if user has permission to access hotel GST report
    const userRole = session.user.role
    const userBusinessUnit = session.user.businessUnit
    
    const allowedRoles = ['super_admin', 'owner', 'hotel_manager']
    const isAuthorized = allowedRoles.includes(userRole) || 
                         (userRole === 'hotel_reception' && userBusinessUnit === 'hotel')
    ...
    return (
        <div className="flex-1 space-y-4 p-8 pt-6">
            <GSTReportComponent businessUnit="hotel" unitName="Hotel" />
        </div>
    )
}
