import HybridBillingPage from "@/components/billing/HybridBillingPage"

export default function BillingPage() {
  return <HybridBillingPage />
}
  ...
  const exportToJSON = () => {
    // Export only database bills since imported bills are now saved to database
  ...
  const importFromJSON = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (isValid) {
          // Import bills to database using createBill action
          ...
          for (const bill of billsToImport) {
            try {
              // Prepare bill data for database import
              const billData = {
              ...
              if (result.success) {
                ...
                errorCount++;
                errors.push(`Bill ${bill.billNumber || 'Unknown'}: ${result.error}`);
              }
            } catch (billError) {
              errorCount++;
              errors.push(`Bill ${bill.billNumber || 'Unknown'}: ${billError instanceof Error ? billError.message : 'Unknown error'}`);
            }
          }

          // Show results
          if (successCount > 0) {
            alert(`Successfully imported ${successCount} bill(s) to database.${errorCount > 0 ? ` ${errorCount} bills failed to import.` : ''}`);
            
            // Refresh bills from database to show the imported data
            const unitParam = searchParams?.get('unit') || undefined;
            const userBusinessUnit = session?.user?.businessUnit || undefined;
            const effectiveUnit = unitParam === 'all' ? undefined : unitParam || userBusinessUnit;
            await loadBills(effectiveUnit);
            
            // Clear imported bills state since they're now in the database
            setImportedBills([]);
          } else {
            alert(`Failed to import bills. Errors:\n${errors.join('\n')}`);
          }
        } else {
  ...
  // Use only database bills since imported bills are now saved to database
