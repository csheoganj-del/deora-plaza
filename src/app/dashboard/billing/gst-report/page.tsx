...
export default async function CafeGSTReportPage() {
    ...
    // Check if user has permission to access cafe GST report
    const userRole = session.user.role
    const userBusinessUnit = session.user.businessUnit
    
    const allowedRoles = ['super_admin', 'owner', 'cafe_manager']
    const isAuthorized = allowedRoles.includes(userRole) || 
                         (userRole === 'waiter' && userBusinessUnit === 'cafe')
    ...
    return (
        <div className="flex-1 space-y-4 p-8 pt-6">
            <GSTReportComponent businessUnit="cafe" unitName="Cafe" />
        </div>
    )
}
