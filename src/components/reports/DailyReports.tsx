  ...
  const downloadReport = async (format: 'pdf' | 'excel') => {
    try {
      ...
      if (format === 'pdf') {
        // Create PDF content
        const reportContent = `
Bloom Cafe - ${timePeriod.charAt(0).toUpperCase() + timePeriod.slice(1)} Report
...
System Downtime: ${report.issues.systemDowntime} minutes
  ...
  const emailReport = async () => {
    try {
      ...
      const emailSubject = `Bloom Cafe ${timePeriod.charAt(0).toUpperCase() + timePeriod.slice(1)} Report - ${report.businessUnit} - ${report.period}`;
      const emailBody = `
