...
export default function HybridBillingPage() {
  ...
  // Guard against null numeric values from incomplete DB records
    ...
    <div className="space-y-8 pb-20">
                ...
                {filteredBills.map((bill) => (
                  <div key={bill.id} className="grid grid-cols-[auto_1.5fr_1fr_1fr_1fr_1fr_1fr_auto] gap-4 p-4 text-sm text-white/80 border-b border-white/5 hover:bg-white/5 transition-colors items-center group">
                    ...
                    <div className="text-xs">
                      <div className="text-white/80">{bill.createdAt ? format(new Date(bill.createdAt), "dd MMM, yyyy") : '—'}</div>
                      <div className="text-white/40">{bill.createdAt ? format(new Date(bill.createdAt), "hh:mm a") : ''}</div>
                    </div>
                    <div>
                      <div className="text-white">{bill.customerName || "Walk-in"}</div>
                      <div className="text-xs text-white/40">{bill.customerMobile}</div>
                    </div>
                    <div className="text-right font-mono font-bold">₹{(bill.grandTotal ?? 0).toLocaleString()}</div>
                    ...
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-white/70 hover:text-white hover:bg-white/10" onClick={() => handleEdit(bill)}>
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-400 hover:text-blue-300 hover:bg-blue-500/10" onClick={() => handleReprint(bill)}>
                        <Printer className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 hover:text-red-300 hover:bg-red-500/10" onClick={(e) => handleDelete(bill.id, e)}>
      ...
      <Dialog open={showReprint} onOpenChange={setShowReprint}>
        <DialogContent className="max-w-[800px] bg-zinc-900 border-zinc-800 text-zinc-100 p-0 overflow-hidden">
          <DialogTitle className="sr-only">Reprint Bill</DialogTitle>
