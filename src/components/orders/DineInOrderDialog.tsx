...
// ─── Module-level client-side menu cache ────────────────────────────────────
// Survives across dialog open/close cycles. Keyed by businessUnit.
const _menuCache: Record<string, { items: MenuItem[]; ts: number }> = {};
const MENU_CACHE_TTL = 5 * 60 * 1000; // 5 minutes

function getCachedMenu(unit: string): MenuItem[] | null {
  const entry = _menuCache[unit];
  if (entry && Date.now() - entry.ts < MENU_CACHE_TTL) return entry.items;
  return null;
}
function setCachedMenu(unit: string, items: MenuItem[]) {
  _menuCache[unit] = { items, ts: Date.now() };
}
// ─────────────────────────────────────────────────────────────────────────────
...
export function DineInOrderDialog({
  ...
  useEffect(() => {
    ...
    // Load menu items — serve from client cache if available (instant)
    if (businessUnit === 'cafe') {
      const cached = getCachedMenu(businessUnit);
      if (cached) {
        setMenuItems(cached);
      ...
      const cachedDrinks = getCachedMenu('bar_drinks');
      const cachedFood = getCachedMenu('bar_food');
      if (cachedDrinks && cachedFood) {
        setBarDrinks(cachedDrinks);
        setBarFood(cachedFood);
        ...
        import('@/actions/bar').then(({ getBarMenu }) => {
          getBarMenu().then((barMenu) => {
            const drinks = barMenu.drinks.filter((item: MenuItem) => item.isAvailable);
            const food = barMenu.food.filter((item: MenuItem) => item.isAvailable);
            setCachedMenu('bar_drinks', drinks);
            setCachedMenu('bar_food', food);
            setBarDrinks(drinks);
            setBarFood(food);
