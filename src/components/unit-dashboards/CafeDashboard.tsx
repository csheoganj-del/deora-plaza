...
export function CafeDashboard() {
  ...
  const loadCafeMetrics = () => {
    setLoading(true);
    
    // Mock data for cafe metrics
    const mockMetrics: CafeMetrics = {
      dailyRevenue: 3450.75,
      ordersToday: 127,
      activeCustomers: 45,
      popularItems: [
        { name: "Cappuccino", count: 34, revenue: 136.00 },
        { name: "Latte", count: 28, revenue: 140.00 },
        { name: "Croissant", count: 22, revenue: 66.00 },
        { name: "Espresso", count: 19, revenue: 38.00 },
        { name: "Sandwich", count: 15, revenue: 75.00 }
      ],
      staffOnDuty: 4,
      inventoryAlerts: 3,
      averageOrderValue: 27.17,
      peakHours: [
        { hour: "7:00 AM", orders: 12 },
        { hour: "8:00 AM", orders: 28 },
        { hour: "9:00 AM", orders: 35 },
        { hour: "12:00 PM", orders: 18 },
        { hour: "1:00 PM", orders: 15 },
        { hour: "3:00 PM", orders: 19 }
      ]
    };
              ...
              {metrics.popularItems.map((item, index) => (
                <div key={index} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-[#9CA3AF]">{formatCurrency(item.revenue / item.count)} avg</p>
                  </div>
                </div>
              ))}
            </div>
