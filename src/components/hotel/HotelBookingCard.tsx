...
import { Calendar, Clock, Bed, Phone, IndianRupee, MoreHorizontal, FileText } from "lucide-react"
...
export function HotelBookingCard({ booking, settings, onEdit, onDelete, onViewBill, onPrintBill }: HotelBookingCardProps) {
    ...
    const getStatusStyle = (status: string) => {
        switch (status.toLowerCase()) {
            ...
            case 'checked-in': return "bg-[#EDEBFF]/30 text-[#6D5DFB] border-[#EDEBFF]/40"
            case 'checked-out': return "bg-[#F1F5F9] text-[#111827] border-[#E5E7EB]"
    ...
    return (
        <div className="premium-card">
            <div className="p-8 border-b border-[#E5E7EB] p-3 sm:p-4 bg-gradient-to-r from-[#6D5DFB]/10 to-[#EDEBFF]/10 border-b border-[#F1F5F9] pb-2 sm:pb-3">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <h3 className="font-bold text-base sm:text-lg text-[#111827] leading-tight truncate" title={booking.guestName}>
                            {booking.guestName}
                        </h3>
                    </div>
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={handlePrintBill}>
            ...
            <div className="p-8 p-3 sm:p-4 space-y-2 sm:space-y-3 flex-grow">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    <div className="h-8 sm:h-10 w-8 sm:w-10 text-xs font-bold rounded-full bg-[#EDEBFF] text-[#5B4EE5] flex items-center justify-center shrink-0" aria-hidden="true">
                        {booking.guestName.slice(0, 2).toUpperCase()}
                    </div>
                    <div className="min-w-0">
                        <p className="font-semibold text-[#111827] truncate text-sm sm:text-base">{booking.guestName}</p>
                        <div className="flex items-center text-xs sm:text-sm text-[#9CA3AF]">
                            <Phone className="h-3 w-3 mr-1" aria-hidden="true" />
                            <span className="truncate">{booking.guestMobile}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center text-xs sm:text-sm text-[#6B7280] border-t border-[#F1F5F9] pt-1 sm:pt-2 mt-1 sm:mt-2">
                    <Bed className="h-4 w-4 mr-2 text-[#9CA3AF]" aria-hidden="true" />
