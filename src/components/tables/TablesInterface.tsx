                        ...
                        {canSeeAllTables && (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                                    activeTab === 'all'
                                        ? 'bg-[#F2B94B]/20 text-[#F2B94B] border border-[#F2B94B]/30 shadow-lg'
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="bg-black/30 border border-white/20 rounded-xl pl-9 pr-4 py-2 text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-[#F2B94B]/50 w-full md:w-48 transition-colors"
                            />
                        </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4">
                    {filteredTables.map(table => {
                        ...
                        return (
                            ...
                            ${isSelected ? 'shadow-[0_0_20px_rgba(var(--primary-rgb),0.2)]' : 'hover:bg-black/60'}
                        `}
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div className="px-3 py-1.5 rounded-lg bg-white/10 border border-white/10">
                                        <span className="text-base font-bold text-white tracking-wide">{table.tableNumber}</span>
                                    </div>
                                    <div
                                        onClick={(e) => { e.stopPropagation(); toggleSelectTable(table.id); }}
                                        className={`h-5 w-5 rounded border flex items-center justify-center transition-colors ${
                                            isSelected ? 'bg-primary border-primary' : 'border-white/40 hover:border-white/60 bg-black/20'
                                        }`}
                                    >
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between text-xs">
                                        ...
                                        <span className="text-white font-semibold">{table.capacity} P</span>
                                    </div>
                                ...
                                {/* Hover Actions */}
                                <div className="absolute inset-0 bg-black/70 backdrop-blur-sm rounded-2xl flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity p-2">
                                    {table.status === 'occupied' ? (
