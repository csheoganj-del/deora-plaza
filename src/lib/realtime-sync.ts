...
class RealtimeSyncManager {
  ...
  private reconnectAttempts = new Map<string, number>() // Per-channel counter
  ...
  subscribe(
    ...
    channel.subscribe((status) => {
      ...
      if (status === 'SUBSCRIBED') {
        this.reconnectAttempts.set(id, 0) // Reset only this channel's counter
  ...
  private handleReconnect(
    ...
    if (attempts >= this.maxReconnectAttempts) {
      console.error(
        `[RealTimeSync] Max reconnection attempts reached for ${id}.`,
        'This usually means the Supabase URL or Anon Key in .env.local is invalid or unreachable.',
    ...
    console.log(`[RealTimeSync] Reconnecting ${id} in ${delay}ms (attempt ${attempts + 1}/${this.maxReconnectAttempts})`)
