...
export async function queryDocuments(
  ...
  try {
    ...
    const { data, error } = await query;
    if (error) {
      const errDetail = { message: error.message, code: error.code, hint: error.hint, details: error.details }
      console.error(`Supabase error querying documents from ${collectionName}:`, JSON.stringify(errDetail))
      throw error;
    }
    return data || [];
  } catch (error) {
    const errMsg = error instanceof Error
      ...
      : (typeof error === 'object' && error !== null)
        ? JSON.stringify({ message: (error as any).message, code: (error as any).code, hint: (error as any).hint })
        : String(error)
    console.error(`Error querying documents from ${collectionName}: `, errMsg)
    return [];
  }
}
