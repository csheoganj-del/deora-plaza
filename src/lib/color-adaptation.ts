  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        const textSecondary = isDark ? 'rgba(255, 255, 255, 0.8)' : 'rgba(0, 0, 0, 0.8)';
...
function generateColorsFromAnalysis(analysis: BackgroundAnalysis): AdaptiveColors {
  ...
  if (isLight) {
    ...
    textSecondary = brightness < 0.2 ? 'rgba(255, 255, 255, 0.85)' : 'rgba(249, 250, 251, 0.8)';
  ...
  return {
    ...
    cardBorder,
    buttonBackground,
    buttonText: isLight ? '#ffffff' : '#ffffff',
    inputBackground,
    inputBorder,
    shadowColor
  };
