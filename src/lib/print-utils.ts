...
function generateHotelReceiptHTML(booking: HotelBooking | any, settings?: any, type: 'advance' | 'full' = 'full'): string {
  ...
  const businessName = hName || settings?.name || settings?.businessName || settings?.business_name || "Bloom Cafe & Restaurant";
  ...
  const businessEmail = hEmail || settings?.email || settings?.businessEmail || settings?.business_email || "hello@bloomcafe.com";
...
function generateGardenReceiptHTML(booking: GardenBooking, settings?: any): string {
  ...
  const businessName = gardenName || settings?.name || "Bloom Cafe & Restaurant";
  ...
  const businessEmail = settings?.email || settings?.gardenEmail || "hello@bloomcafe.com";
