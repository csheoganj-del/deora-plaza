...
export function printGardenReceipt(booking: any, paymentType: "advance" | "full" = "advance") {
  // Check if trying to print final receipt but payment is not complete
  if (paymentType === "full" && booking.paymentStatus !== "completed") {
    // Exception: If we're printing a specific payment receipt that is marked as final, allow it
    const isSpecificPaymentReceipt = booking.receiptNumber && 
      booking.payments?.some((p: any) => p.receiptNumber === booking.receiptNumber && p.type === "final");
      
    if (!isSpecificPaymentReceipt) {
...
<head>
  <meta charset="UTF-8">
  <title>Receipt ${booking.receiptNumber || 'Unknown'}</title>
  <style>
    @media print {
 ...
 * Print hotel receipt
 */
export function printHotelReceipt(booking: any, paymentType: "advance" | "full" = "advance") {
  // Check if trying to print final receipt but payment is not complete
  if (paymentType === "full" && booking.paymentStatus !== "completed") {
    // Exception: If we're printing a specific payment receipt that is marked as final, allow it
    const isSpecificPaymentReceipt = booking.receiptNumber && 
  ...
  // Import the server-compatible receipt component dynamically
  import('../components/hotel/HotelReceiptServer').then(({ HotelReceiptServer }) => {
    import('react-dom/server').then((ReactDOMServer) => {
      import('react').then((React) => {
        const receiptHTML = ReactDOMServer.renderToString(
          React.createElement(HotelReceiptServer, {
            booking,
            receiptNumber: booking.receiptNumber,
            paymentType,
            businessSettings: null, // Will be fetched in component
